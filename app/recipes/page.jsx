...
export default async function RecipeList() {
	...
	return (
		<main className="body w-full py-20 flex flex-col items-center">
			...
			<ul className="w-full max-w-1/2 min-w-87.5">
				{data.map((recipe) => (
					<li key={recipe.slug}>
						<Link
							href={`/recipes/${recipe.slug}`}
							className="m-5 flex flex-col gap-1 rounded-2xl border border-foreground/10 bg-foreground/5 px-5 py-4 hover:bg-foreground/10 transition-all"
						>
